...
import os
import json
from auth_example import TwitterAuth
import time
import sys
...
class FileDumperListener(StreamListener):
	...
	def status(self):
		now=datetime.now()
		if (now-self.last).total_seconds()>900:
			print("%s - %i tweets, %i limits, %i errors in previous 15 minutes."%(now,self.tweetCount,self.limitCount,self.errorCount))
			self.tweetCount=0
			self.limitCount=0
			self.errorCount=0
			self.last=now
...
if __name__ == '__main__':
	while True:
		try:
			...
			stream.filter(locations=[-124.7625, 24.5210, -66.9326, 49.3845])
